...
export default {
    ...
    analyzeInfrastructure: async (code: string, modelId: string): Promise<any> => {
        ...
        try { return JSON.parse(extractJson(res.text || '{}')); } catch (e) { return { costEstimate: 'N/A', securityStatus: 'unknown' }; }
    },
    ...
    analyzeTerminalError: async (errorLog: string, modelId: string): Promise<{ issue: string, fix: string, command: string }> => {
        const res = await getApiResponse({ contents: [{ parts: [{ text: errorLog }] }], model: modelId, responseMimeType: 'application/json' });
        try { return JSON.parse(extractJson(res.text || '{}')); } catch (e) { return { issue: 'Failed', fix: 'Retry', command: 'clear' }; }
    },
