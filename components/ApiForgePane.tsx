...
const ApiStudioPane: React.FC<ApiStudioPaneProps> = (props) => {
    ...
    const ClientView = () => {
        ...
        const [response, setResponse] = useState<{ status?: number; statusText?: string; time?: number; size?: string; data?: any; } | null>(null);
        ...
        const handleSend = async () => {
            ...
            try {
                ...
                setResponse({ status: res.status, statusText: res.statusText, time: Math.round(endTime - startTime), size: (size / 1024).toFixed(2) + ' KB', data });
            } catch (error: any) {
                setResponse({ status: 0, statusText: 'Error', time: 0, size: '0 KB', data: { error: error.message } });
            } finally {
                setIsLoading(false);
            }
        };
        ...
        return (
            <div className="flex flex-col flex-1 overflow-hidden">
                <div className="p-3 bg-[var(--color-background-secondary)] border-b border-white/5">
                    <div className="flex gap-2">
                        <select value={method} onChange={(e) => setMethod(e.target.value as HttpMethod)} className={`bg-black/40 border border-white/10 rounded-lg px-2 py-1.5 text-xs font-black focus:outline-none ${method === 'GET' ? 'text-green-400' : 'text-blue-400'}`}>
                            {['GET', 'POST', 'PUT', 'DELETE', 'PATCH'].map(m => <option key={m} value={m}>{m}</option>)}
                        </select>
                        <input type="text" value={url} onChange={(e) => setUrl(e.target.value)} placeholder="https://api.example.com/v1" className="flex-1 bg-black/40 border border-white/10 rounded-lg px-3 py-1.5 text-sm focus:outline-none focus:border-[var(--color-accent)] font-mono text-gray-300"/>
                        ...
                        <div className="space-y-2">
                            { (activeTab === 'params' ? params : headers).map((item, index) => (
                                <div key={index} className="flex items-center gap-2 group">
                                    <input type="checkbox" checked={item.enabled} onChange={() => {}} className="h-3.5 w-3.5 rounded bg-gray-700 border-gray-600 accent-[var(--color-accent)]"/>
                                    ...
                                    <button className="text-gray-500 hover:text-red-400 opacity-0 group-hover:opacity-100 p-1"><Trash2 size={14}/></button>
                                </div>
                            ))}
                        </div>
                    )}
                    {activeTab === 'body' && <textarea value={body} onChange={(e) => setBody(e.target.value)} className="w-full h-full bg-black/30 border border-white/10 rounded-xl p-4 font-mono text-xs text-gray-300 focus:border-[var(--color-accent)] outline-none resize-none" />}
                </div>
                ...
                <div className="h-2/5 border-t border-white/5 flex flex-col min-h-[200px]">
                    <div className="flex justify-between items-center py-2 px-4 border-b border-white/5 bg-[#0f111a]">
                        ...
                        {response && <div className="flex gap-4 text-[9px] font-black uppercase"><span className={`flex items-center gap-1 ${response.status && response.status < 300 ? 'text-emerald-400' : 'text-rose-400'}`}>{response.status} {response.statusText}</span><span className="text-gray-500 flex items-center gap-1"><Clock size={10}/> {response.time}ms</span><span className="text-gray-500 flex items-center gap-1"><Database size={10}/> {response.size}</span></div>}
                    </div>
                    <div className="flex-1 bg-black/20 relative overflow-hidden">
                        {isLoading ? <div className="absolute inset-0 flex items-center justify-center"><RefreshCw className="animate-spin text-emerald-400" size={24} /></div>
                        : response ? <pre className="p-4 text-[11px] font-mono h-full overflow-auto text-emerald-100/80 custom-scrollbar">{typeof response.data === 'object' ? JSON.stringify(response.data, null, 2) : response.data}</pre>
                        : <div className="h-full flex items-center justify-center text-gray-600 italic text-xs">Awaiting request...</div>}
                    </div>
                </div>
            </div>
        );
