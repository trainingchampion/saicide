...
const WelcomeScreen: React.FC<WelcomeScreenProps> = ({ 
    ...
    const BLUEPRINTS = [
        ...
        { 
            ...
            color: 'text-blue-600', 
            bg: 'bg-blue-100',
            action: () => onAIAppGen?.("Let's start a new architectural discussion.", 'chat')
        },
    ];
        ...
        <div ref={scrollContainerRef} className="h-full w-full bg-[var(--color-background)] relative overflow-y-auto custom-scrollbar flex flex-col p-8 md:p-16 text-[var(--color-text-primary)]">
            ...
            <div className="relative z-10 max-w-5xl mx-auto w-full">
                    ...
                    <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-5 gap-6">
                        {BLUEPRINTS.map(bp => (
                            <button 
                                key={bp.id} 
                                onClick={() => bp.action?.()}
                                className="text-left bg-white sketch-border sketch-shadow p-6 group hover:-translate-y-2 transition-all relative"
                            >
                ...
                <div ref={footerActionsRef} style={{ willChange: 'transform' }}>
                    <div className="mt-20 pt-16 border-t-2 border-dashed border-gray-200 flex flex-wrap gap-12 justify-center">
