...
import { ICONS, MOCK_EXTERNAL_AGENTS } from '../constants';
import { THEMES } from '../themes';
import { Check, Plus, ChevronDown, ChevronRight, Bot, Server, Zap, ShoppingBag, Key, Save, X, Trash2, Power, BrainCircuit, Code2, CreditCard, Star } from 'lucide-react';
...
const SettingsPane: React.FC<SettingsPaneProps> = ({ 
  ...
  const [agents, setAgents] = useState<ExternalAgent[]>(MOCK_EXTERNAL_AGENTS);
  ...
  const providers: LLMProviderConfig[] = [
    { id: 'google', name: 'Google Gemini', isConfigured: !!apiKeys['google'] },
    { id: 'anthropic', name: 'Anthropic Claude', isConfigured: !!apiKeys['anthropic'] },
    { id: 'openai', name: 'OpenAI GPT', isConfigured: !!apiKeys['openai'] },
  ];
        ...
        <section className="space-y-4 pt-2">
            ...
            <div className="space-y-2">
                ...
                <div className="bg-[var(--color-background-tertiary)]/30 border border-[var(--color-border)] rounded-lg divide-y divide-[var(--color-border)]">
                    {agents.map(agent => (
                        <div key={agent.id} className="flex items-center justify-between p-2.5 cursor-pointer hover:bg-[var(--color-background-tertiary)]/50" onClick={() => toggleAgent(agent.id)}>
                            <div className="flex items-center gap-2">
                                <Bot className="w-4 h-4 text-[var(--color-text-secondary)]" />
                                <span className={`text-sm ${agent.isEnabled ? 'text-gray-200' : 'text-gray-500'}`}>{agent.name}</span>
                            </div>
                            {agent.isEnabled && <Check className="w-4 h-4 text-green-400" />}
                        </div>
                    ))}
                </div>
            </div>
