        ...
        setTimeout(() => {
            onSetPolicies(currentPolicies => {
                return currentPolicies.map(policy => {
                    ...
                    if (policy.enabled) {
                        const foundIssues = securityIssues.filter(issue => policy.issueTypes.includes(issue.severity));
                        newViolationsCount = foundIssues.length;
                    }
                        ...
                        <div className="p-4 overflow-y-auto flex-1 custom-scrollbar">
                            {activeTab === 'overview' ? (
                                ...
                                <div className="space-y-1">
                                    <p className="text-xs text-gray-500 mb-3 uppercase font-bold tracking-wider">Detailed Controls</p>
                                    {INFRASTRUCTURE_SECURITY_CONTROLS.map(control => (
